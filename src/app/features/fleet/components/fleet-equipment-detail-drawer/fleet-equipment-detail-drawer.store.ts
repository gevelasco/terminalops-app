...
export interface FleetEquipmentDetailDrawerHostInputs {
  equipment: Equipment;
  unitOptions: ToSelectOption[];
  unitCatalog: Unit[];
  onRoute: boolean;
...
@Injectable()
export class FleetEquipmentDetailDrawerStore {
  private readonly doc = inject(DOCUMENT);
  private readonly toast = inject(ToastService);
  private readonly session = inject(SessionService);

  private hostCallbacks: FleetEquipmentDetailDrawerHostCallbacks | null = null;

  private readonly equipmentSource = signal<Equipment | null>(null);
  private readonly unitOptionsSource = signal<ToSelectOption[]>([]);
  ...
  readonly unitOptions = computed(() => this.unitOptionsSource());
  ...
  bindHost(
    inputs: FleetEquipmentDetailDrawerHostInputs,
    callbacks: FleetEquipmentDetailDrawerHostCallbacks,
  ): void {
    this.hostCallbacks = callbacks;
    this.equipmentSource.set(inputs.equipment);
    this.unitOptionsSource.set(inputs.unitOptions);
    this.unitCatalogSource.set(inputs.unitCatalog);
  ...
  readonly unitLabel = computed(() => {
    ...
    const unit = this.unitCatalog().find((u) => u.id === id);
    return unit ? formatUnitTrailerOperationalId(unit) : id;
  });
