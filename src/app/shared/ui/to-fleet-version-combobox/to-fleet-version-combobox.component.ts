...
@Component({
  selector: 'to-fleet-version-combobox',
  standalone: true,
          ...
          <ul [id]="listId" class="dr-maneuver-combobox__list" role="listbox">
            @if (suggestions().length > 0) {
              ...
              <li class="dr-maneuver-combobox__hint" role="presentation">
                Usar «{{ inputText().trim() }}» como versión nueva
              </li>
            }
          </ul>
        }
...
export class ToFleetVersionComboboxComponent {
  ...
  readonly versionName = model('');

  readonly inputId = `fleet-version-combobox-${++seq}`;
  ...
  onInput(ev: Event): void {
    ...
    this.versionName.set(text);
    this.open.set(true);
  }

  onBlur(): void {
    this.open.set(false);
    this.versionName.set(this.inputText());
  }

  onPickPointerDown(name: string, ev: PointerEvent): void {
    ev.preventDefault();
    this.inputText.set(name);
    this.versionName.set(name);
    this.open.set(false);
  }
}
